...
module.exports = (BasePlugin) ->
  ...
  class NoSqlPlugin extends BasePlugin
    ...
    config:
      collectionDefaults:
        connectionString: process.env.NOSQL_FILE
    ...
    fetchNosqlCollection: (collectionConfig, next) ->
      NoSql.load collectionConfig.nosqlFile, collectionConfig.nosqlBinary

      NoSql.on "load" (loaded) ->
        docpad.log('info', "NoSql loaded  #{collectionConfig.collectionConfig.nosqlFile}")

      NoSql.on "error" (err, source) ->
        docpad.log('debug', "Fail to laod nosql file  #{collectionConfig.collectionConfig.nosqlFile}")
        return next err if err


      fnMap = (doc) ->
        return doc if collectionConfig.query()
i     fnCallback = (err, docs) ->
          next err, docs
